...
export default function LandingCTASection() {
  ...
  return (
    <section className="w-full py-28 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #050810 0%, #0f172a 40%, #1a0a2e 70%, #050810 100%)' }}>
      <div className="absolute top-0 left-0 right-0 h-px bg-stella-gradient opacity-40" aria-hidden="true" />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-8 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Link to="/app" className="px-8 py-4 font-semibold rounded-xl transition-all duration-300 text-lg focus:outline-none focus:ring-2 focus:ring-stella-500/50 text-white" style={{ background: 'linear-gradient(135deg,#3b82f6,#a855f7)' }} aria-label="Join the developer beta">Join Developer Beta →</Link>
        </div>
        <div>
          <form onSubmit={e => { e.preventDefault(); if (email) setSubmitted(true) }} className="max-w-md mx-auto" aria-label="Newsletter signup">
            {submitted ? (
              ...
              <div className="flex items-center bg-surface-900/80 border border-surface-600 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-stella-500/50 focus-within:border-stella-500/50 transition-all duration-200">
                ...
                <span className="pr-3 text-stella-400 font-mono animate-pulse" aria-hidden="true">_</span>
                <button type="submit" className="px-5 py-3.5 bg-stella-500 hover:bg-stella-600 text-white text-sm font-semibold transition-colors duration-200 focus:outline-none" aria-label="Subscribe">Subscribe</button>
              </div>
            )}
          </form>
        </div>
