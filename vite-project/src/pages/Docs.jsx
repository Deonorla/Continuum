...
function StatCard({ label, value }) {
  return (
    <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
      ...
      <div className="mt-2 text-sm font-semibold text-white truncate">{value}</div>
    </div>
  );
}
...
function PlainLanguageCard({ text }) {
  if (!text) return null;
  return (
    <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 px-5 py-4 text-sm leading-7 text-white/70">
      {text}
    </div>
  );
}
...
function TakeawayPanel({ items }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5 space-y-2">
      <div className="text-xs uppercase tracking-[0.22em] text-cyan-300">Key Takeaways</div>
    ...
    <div className="grid gap-3 md:grid-cols-2">
      {items.map((item, i) => (
        <div key={i} className="rounded-2xl bg-white/5 border border-white/10 p-4">
          ...
          <div className="mt-1 text-sm text-white/55 leading-6">{item.body || item}</div>
        </div>
      ))}
    </div>
  );
...
function CodeCard({ code }) {
  if (!code) return null;
  return (
    <pre className="rounded-2xl border border-white/10 bg-black/40 p-5 overflow-x-auto text-xs text-cyan-200 font-mono leading-relaxed">
      {code}
    </pre>
  );
}
...
function StepList({ title, steps }) {
  ...
  return (
    <div className="space-y-3">
      {title && <div className="text-xs uppercase tracking-[0.22em] text-cyan-300">{title}</div>}
      {steps.map((step, i) => (
        <div key={i} className="flex items-start gap-4 rounded-2xl bg-white/5 border border-white/10 p-4">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-flowpay-500/20 text-xs font-bold text-cyan-300">
            {i + 1}
          </div>
          <div className="text-sm text-white/65 leading-6">{step.body || step}</div>
        </div>
      ))}
    </div>
  );
          ...
          {headers?.length > 0 && (
            <thead>
              <tr className="border-b border-white/10">
                {headers.map((h, i) => <th key={i} className="px-4 py-3 text-left text-xs font-semibold text-white/50 uppercase tracking-wider">{h}</th>)}
              </tr>
            </thead>
          )}
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className="border-b border-white/5 last:border-0">
                {(Array.isArray(row) ? row : Object.values(row)).map((cell, j) => (
                  <td key={j} className="px-4 py-3 text-white/65 font-mono text-xs">{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
...
function ExplorerLinks({ items }) {
  ...
  return (
    <div className="space-y-2">
      <div className="text-xs uppercase tracking-[0.22em] text-cyan-300">Explorer Links</div>
      <div className="flex flex-wrap gap-2 mt-2">
        {items.map((item, i) => (
          <a key={i} href={item.url} target="_blank" rel="noopener noreferrer"
            className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs text-white/65 hover:text-white hover:border-white/20 transition-colors">
            {item.label}
          </a>
        ))}
      </div>
    ...
    <div className="space-y-3">
      {items.map((item, i) => (
        <div key={i} className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="mt-2 text-sm text-white/55 leading-6">{item.answer}</div>
        </div>
      ))}
    </div>
  );
