...
const INITIAL_NODES = [
    { id: 'client', label: 'Client', Icon: User, type: 'client', x: 80, y: 200, color: 'stella' },
...
const GraphNode = ({ node, isSelected, onSelect, onDragStart, isDragging }) => {
    const colors = {
        stella: 'from-stella-500 to-stella-600 border-stella-400 shadow-glow',
    ...
    return (
        ...
        >
            ...
            <circle
                r="30"
                className={`fill-current ${colors[node.color]?.includes('stella') ? 'text-stella-500' :
    ...
    return (
        <div className="absolute bottom-4 left-4 right-4 glass rounded-xl p-4 z-10">
            <div className="grid grid-cols-4 gap-4 text-center">
                <div>
                    <div className="text-xl font-bold text-stella-400">{nodes.length}</div>
