...
const TESTIMONIALS = [
  {
    ...
    quote: "Replaced our billing layer with Stella's Stream Engine in 4 days. Agents now negotiate USDC payment mode dynamically with zero manual config.",
  },
  {
    user: '@rwa_builder',
    ...
    <section className="w-full bg-surface-950 py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="space-y-3 mb-16">
          <p className="text-stella-400 text-sm font-semibold uppercase tracking-widest font-mono">Loved by builders</p>
          <h2 className="text-4xl lg:text-5xl font-bold text-white">What builders are saying.</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {TESTIMONIALS.map((t, i) => (
            <div key={i} className="bg-surface-900 border border-surface-700 rounded-xl overflow-hidden hover:border-stella-500/30 hover:shadow-glow transition-all duration-300">
              <div className="flex items-center gap-2 px-4 py-2.5 bg-surface-800 border-b border-surface-700">
                ...
                <span className="ml-2 text-xs font-mono text-stella-400">{t.user}</span>
              </div>
