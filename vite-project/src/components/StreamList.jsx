...
const FilterTab = ({ label, count, active, onClick }) => (
  ...
  >
    {label}
    {count > 0 && (
      <span className={`ml-2 px-1.5 py-0.5 rounded text-xs ${active ? 'bg-stella-500/30' : 'bg-white/10'}`}>
        {count}
      </span>
    )}
  </button>
...
const SortDropdown = ({ value, onChange }) => {
  ...
  return (
    <select
      ...
      className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white/80 focus:outline-none focus:border-stella-500"
    >
      {options.map(opt => (
