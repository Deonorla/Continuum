...
export class StellaSDK {
    ...
    constructor(config: StellaConfig) {
        ...
        if (!this.wallet && !this.adapter) {
            throw new Error("StellaSDK requires either `privateKey` or `adapter`.");
        }
        ...
        // For Hackathon, let's assume config.apiKey is the Stella key, but we might pass a separate 'geminiKey' in config?
    ...
    public emergencyStop() {
        this.isPaused = true;
        console.warn("[StellaSDK] 🚨 EMERGENCY STOP ACTIVATED. All payments paused.");
    }

    public resume() {
        this.isPaused = false;
        console.log("[StellaSDK] ✅ System Resumed.");
    }
    ...
    public async selectPaymentMode(estimatedRequests: number): Promise<'direct' | 'stream'> {
        ...
        console.log(`[StellaSDK] 🤖 Gemini Analysis: ${decision.reasoning}`);
    ...
    public async makeRequest(url: string, options: AxiosRequestConfig = {}): Promise<AxiosResponse> {
        if (this.isPaused) {
            throw new Error("StellaSDK is paused due to Emergency Stop.");
        }
        ...
        try {
            ...
            if (cachedStream) {
                ...
                // Or just if > 0.
                if (remaining <= 0n) {
                    console.log("[StellaSDK] Cached stream depleted. Clearing cache...");
                    ...
                    if (remaining < threshold) {
                        console.log("[StellaSDK] Stream balance low (<10%). Triggering renewal...");
                        this.activeStreams.delete(host);
                        // We delete it so the request goes through without header, gets 402, and creates NEW stream.
                        // This is "Lazy Renewal".
                    } else {
                        (headers as any)['X-Stella-Stream-ID'] = cachedStream.streamId;
                    }
                }
            }
            ...
            if (axios.isAxiosError(error) && error.response && error.response.status === 402) {
                ...
                if (cachedStream) {
                    console.log("[StellaSDK] Cached stream failed. Clearing cache and renegotiating...");
                    this.activeStreams.delete(host);
                }

                console.log("[StellaSDK] 402 Payment Required intercepted. Negotiating...");
    ...
    private async handlePaymentRequired(url: string, options: AxiosRequestConfig, response: AxiosResponse): Promise<AxiosResponse> {
        ...
        const mode = headers['x-stella-mode']; // 'streaming' or 'hybrid' (not robust yet, server sends fixed 'streaming' usually)
        // Let's assume server might send 'hybrid' or we decide based on capability.
        const sessionEndpoint =
            headers['x-stella-session-endpoint']
            || response.data?.requirements?.sessionEndpoint;

        const rate = headers['x-stella-rate']; // amount per second or per request
        const paymentTokenAddress = headers['x-stella-token'];
        const headerTokenDecimals = Number(headers['x-stella-token-decimals']);
        ...
        const contractAddress = headers['x-stella-contract'];
        const recipientAddress =
            headers['x-stella-recipient'] ||
            response.data?.requirements?.recipient;

        if (!contractAddress) {
            throw new Error("Missing X-Stella-Contract header in 402 response");
        }
        if (!recipientAddress) {
            throw new Error("Missing X-Stella-Recipient in 402 response");
        }
        ...
        if (sessionEndpoint || headers['x-stella-settlement'] === 'soroban-sac') {
            selectedMode = 'stream';
        }
        ...
        if (mode !== 'streaming' && mode !== 'hybrid') { // Server usually sends 'streaming'
            ...
            throw new Error(`StellaSDK currently only supports 'streaming' or 'hybrid' mode. Got: ${mode}`);
        }
        ...
        try {
            ...
            console.error(`[StellaSDK] Spend Declined: ${e.message}`);
        ...
        console.log(`[StellaSDK] Initiating Stream: ${formatPaymentAmount(totalAmount, tokenDecimals)} ${tokenSymbol} for ${duration}s`);
        ...
        console.log(`[StellaSDK] Stream #${streamData.streamId} created. Retrying request...`);

        const retryOptions = {
            ...options,
            headers: {
                ...options.headers,
                'X-Stella-Stream-ID': streamData.streamId
            }
        };
    ...
    public async createStream(
        ...
        const stella = this.adapter ? null : new Contract(contractAddress, this.MIN_ABI, this.wallet || this.provider);
        ...
        if (!paymentTokenAddress) {
            if (this.adapter?.readContract) {
                ...
                try {
                    paymentTokenAddress = await stella!.paymentToken();
                } catch {
        ...
        if (this.adapter) {
            console.log(`[StellaSDK] Approving ${this.tokenSymbol} through adapter...`);
            await this.adapter.approveToken(paymentTokenAddress, contractAddress, amount);
            console.log("[StellaSDK] Approved.");
            console.log(`[StellaSDK] Creating stream to ${resolvedRecipient}...`);
        ...
        if (!this.wallet) {
            throw new Error("StellaSDK wallet is not configured");
        }
        ...
        try {
            ...
            console.warn("[StellaSDK] Unable to read token allowance through RPC. Falling back to unconditional approval.");
            console.warn(`[StellaSDK] Allowance read error: ${error?.shortMessage || error?.message || error}`);
        }

        if (shouldApprove) {
            console.log(`[StellaSDK] Approving ${this.tokenSymbol}...`);
        ...
        const tx = await stella!.createStream(resolvedRecipient, duration, amount, metadataString);
        const receipt = await tx.wait();

        // Parse event to get ID
        const log = receipt.logs.find((l: any) => {
            try {
                return stella!.interface.parseLog(l)?.name === 'StreamCreated';
            } catch { return false; }
        });

        if (!log) throw new Error("StreamCreated event not found");

        const parsed = stella!.interface.parseLog(log);
    ...
    private async performDirectPayment(
        ...
        if (this.isPaused) throw new Error("StellaSDK is paused.");
        const resolvedRecipient = normalizeRecipientAddress(recipient);
        ...
        console.log(`[StellaSDK] Executing Direct Payment of ${formatPaymentAmount(amount, tokenDecimals)} ${tokenSymbol}`);

        if (this.adapter) {
            const tx = await this.adapter.transferToken(tokenAddress, resolvedRecipient, amount);
            txHash = (tx as { hash?: string }).hash || "";
            if (txHash) {
                console.log(`[StellaSDK] Direct Payment Sent: ${txHash}`);
            }
        } else {
            if (!this.wallet) {
                throw new Error("StellaSDK wallet is not configured");
            ...
            console.log(`[StellaSDK] Direct Payment Sent: ${tx.hash}`);
        }
