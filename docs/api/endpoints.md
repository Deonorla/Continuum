...
| `POST` | `/api/rwa/assets` | mint a verified rental twin and return v2 verification state |
...
Fresh v2 mints return the signed v2 `verificationPayload` plus top-level `verificationStatus`. They do not emit a new legacy verification payload.
