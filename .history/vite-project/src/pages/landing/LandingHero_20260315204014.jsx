...
function CityProjector({ cityIndex, onProject }) {
  ...
  useEffect(() => {
    ...
    if (vec.current.z < 1) onProject({ x, y, city, containerW: sizeW, containerH: sizeH })
    else onProject(null)
...
export default function LandingHero() {
      ...
      if (i < phrase.length) {
        ...
        typingRef.current = setTimeout(() => {
          const erase = () => {
            ...
            else typingRef.current = setTimeout(() => setPhraseIdx(p => (p + 1) % TYPED_PHRASES.length), 300)
          }
  ...
  useEffect(() => {
    const id = setInterval(() => setTxCount(n => n + Math.floor(Math.random() * 3) + 1), 800)
  ...
  return (
    <section className="relative min-h-screen w-full bg-surface-950 overflow-hidden flex flex-col items-center justify-center pt-16">
      <div className="absolute inset-0 bg-grid bg-[size:40px_40px] opacity-[0.03] pointer-events-none" aria-hidden="true" />
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 70% at 50% 55%, rgba(59,130,246,0.08) 0%, transparent 70%)' }} aria-hidden="true" />

      {/* ── Headline ── */}
      <div className="relative z-10 text-center px-4 pt-6 pb-0 animate-slide-up">
        ...
        <p className="text-base text-surface-400 max-w-md mx-auto leading-relaxed">
          AI agents. Streaming payments. Real-world assets. All on Polkadot.
        </p>
      </div>

      {/* ── Globe ── */}
      <div className="relative w-full" style={{ height: 'clamp(300px, 55vw, 72vh)' }} aria-label="Live global payment network" role="img">
        ...
        {projected && (
          <div className="absolute inset-0 pointer-events-none hidden sm:block" style={{ opacity: visible ? 1 : 0, transition: 'opacity 0.35s' }}>
            {(() => {
              const side = projected.x > projected.containerW / 2 ? 'right' : 'left'
              const cardW = 152
              const cardH = 68
              const cardY = Math.max(16, Math.min(projected.y - cardH / 2, projected.containerH - cardH - 16))
              // card anchor point (the tip of the line on the card side)
              const cardAnchorX = side === 'left' ? cardW + 12 : projected.containerW - cardW - 12
                ...
                <>
                  ...
                  <svg className="absolute inset-0 w-full h-full overflow-visible">
                    <line
                      ...
                      strokeDasharray="4 3"
                    />
                    <circle cx={projected.x} cy={projected.y} r="3" fill={projected.city.color} opacity="0.9" />
                  </svg>
                  {/* Card */}
                  <div
                  ...
                  >
                    <div className="flex items-center gap-1.5 mb-1">
                      <span className="w-1.5 h-1.5 rounded-full animate-pulse flex-shrink-0" style={{ background: projected.city.color }} />
                      <span className="font-mono text-[10px] font-bold" style={{ color: projected.city.color }}>{projected.city.type}</span>
                    </div>
                    <p className="font-mono text-xs text-white font-semibold leading-tight">{projected.city.detail}</p>
                    <p className="font-mono text-[10px] text-surface-500 mt-0.5">{projected.city.label}</p>
                  </div>
                </>
              )
            })()}
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 pointer-events-none z-10">
          <div className="flex items-center gap-2 bg-surface-900/80 backdrop-blur border border-surface-700 rounded-full px-4 py-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-success-400 animate-pulse" aria-hidden="true" />
            <span className="font-mono text-xs text-surface-400">txns processed</span>
            <span className="font-mono text-sm font-bold text-flowpay-400 tabular-nums">{txCount.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="absolute bottom-4 left-4 flex flex-col gap-1.5 pointer-events-none z-10">
          {LEGEND.map(l => (
            <div key={l.label} className="flex items-center gap-2 bg-surface-900/80 backdrop-blur border border-surface-700 rounded-lg px-3 py-1.5">
              <span className={`w-2 h-2 rounded-full ${l.color}`} aria-hidden="true" />
              <span className="font-mono text-[10px] text-surface-400">{l.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="absolute bottom-4 right-4 pointer-events-none z-10">
          <span className="font-mono text-[10px] text-surface-600 flex items-center gap-1">
            <svg width="10" height="10" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M15 15l6 6m-6-6v4.8m0-4.8h4.8M9 20.2V15m0 0H4.2M9 15l-6 6M15 4.2V9m0 0h4.8M15 9l6-6M9 9H4.2M9 9V4.2M9 9l-6-6" strokeLinecap="round"/>
            </svg>
            drag to rotate
          </span>
        </div>
      ...
      <div className="relative z-10 text-center px-4 pb-10 space-y-4 animate-fade-in">
        <div className="flex flex-wrap gap-2 justify-center">
          {[
            { label: 'x402 Streaming', cls: 'border-flowpay-500/40 text-flowpay-300 bg-flowpay-500/10' },
            { label: 'RWA NFT Yields', cls: 'border-success-500/40 text-success-300 bg-success-500/10' },
            { label: 'Flash Advance',  cls: 'border-warning-500/40 text-warning-300 bg-warning-500/10' },
          ].map(p => (
            <span key={p.label} className={`text-xs font-mono px-3 py-1 rounded-full border ${p.cls}`}>{p.label}</span>
          ))}
        </div>
        <div className="flex flex-wrap gap-4 justify-center">
          ...
          >
            ...
            <span className="absolute inset-0 bg-white/0 group-hover:bg-white/10 transition-colors duration-300" aria-hidden="true" />
          </Link>
          <a href="#developers"
