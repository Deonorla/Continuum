  ...
  const features = [
    {
      ...
      color: 'text-stella-400', span: 'md:col-span-2',
      extra: (
        <div className="mt-4 rounded-lg bg-surface-900 border border-surface-700 p-3 font-mono text-xs space-y-1">
          <div><span className="text-surface-500">GET</span> <span className="text-stella-300">/api/weather</span></div>
          ...
          <div className="pl-2 text-surface-400">X-Stream-Rate: <span className="text-stella-400">0.0001 {tokenSymbol}/sec</span></div>
    ...
    <section id="use-cases" className="w-full bg-surface-900 py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="space-y-3 mb-16">
          <p className="text-stella-400 text-sm font-semibold uppercase tracking-widest font-mono">Features</p>
          <h2 className="text-4xl lg:text-5xl font-bold text-white">Built for the agent economy.</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-4">
          {features.map((f, i) => (
            <div key={i} className={`bg-surface-800/60 backdrop-blur border border-surface-700 rounded-2xl p-6 hover:border-stella-500/40 hover:shadow-glow transition-all duration-300 ${f.span || ''} ${f.glow || ''}`}>
