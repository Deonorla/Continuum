  ...
  return (
    ...
    >
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between" aria-label="Main navigation">
        ...
        <div className="hidden md:flex items-center gap-8">
          {['Protocol', 'Use Cases', 'Developers'].map((link) => (
            <a
              ...
              className="relative text-surface-200 hover:text-white transition-colors duration-200 text-sm after:absolute after:bottom-[-2px] after:left-0 after:w-0 after:h-[1px] after:bg-flowpay-400 after:transition-all after:duration-300 hover:after:w-full"
            >
              {link}
            </a>
          ))}
        ...
        {/* Mobile hamburger */}
        <button
          className="md:hidden text-surface-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-flowpay-500/50 rounded"
      ...
      {menuOpen && (
        <div className="md:hidden bg-surface-950 border-b border-surface-700 px-4 py-6 flex flex-col gap-4 animate-slide-down">
          ...
          <Link
            to="/app"
            className="mt-2 px-5 py-3 bg-flowpay-500 hover:bg-flowpay-600 text-white font-semibold rounded-lg text-center transition-all duration-300"
            onClick={() => setMenuOpen(false)}
          >
            Launch App
          </Link>
