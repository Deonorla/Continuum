  ...
  return [
    {
      ...
      title: 'Overview',
      eyebrow: 'Start Here',
      summary: 'What Stream Engine is, what problem it solves, and how the pieces fit together.',
      plainEnglish:
      ...
      faqs: [
        {
          ...
          answer: 'No. It handles API payments and rental RWAs. The same stream logic can power machine payments, rental access, and asset-linked revenue flows.',
        },
        {
          question: 'Why keep x402 if streams already exist?',
          answer: 'Because streams solve settlement, not discovery. x402 tells the client what it must pay, who to pay, and which mode is allowed.',
        },
      ],
    },
    {
      ...
      summary: 'The cleanest way to understand the product is to separate the protocol layer, settlement layer, enforcement layer, and asset layer.',
      plainEnglish:
      ...
      tables: [
        {
          ...
          rows: [
            ['x402 / route policy', 'Describe payment requirements', 'What must be paid, to whom, and in which mode?'],
            ['Stream / payment contracts', 'Move and settle value', 'How does value accrue or get claimed?'],
            ['Middleware / compliance', 'Enforce access and safety', 'Should this request or withdrawal be allowed right now?'],
            ['RWA registry / NFT / verification', 'Track asset truth', 'What is this asset, and is its story consistent?'],
          ],
        },
      ],
      faqs: [
        {
          ...
          answer: 'Because it prevents confusion. If you mix negotiation, settlement, access control, and asset ownership together, every bug becomes harder to reason about and every explanation becomes vague.',
        },
      ],
    },
    {
      ...
      title: 'Streaming 101',
      eyebrow: 'Money Flow',
      summary: 'What a payment stream is and why it is better than repeated onchain payments for high-frequency usage.',
      plainEnglish:
...
claimable = (flowRate * elapsed) - amountWithdrawn`,
      ...
      tables: [
        {
          ...
          rows: [
            ['Direct payment', 'Small or infrequent calls', 'Too many signatures when used repeatedly'],
            ['Subscription', 'Fixed predictable access', 'Overpays when usage is uncertain'],
            ['Streaming', 'High-frequency or session-based usage', 'Needs stream lifecycle logic, but has the best economics'],
          ],
        },
      ],
      faqs: [
        {
          ...
          answer: 'No. They only get the portion that has accrued so far. The rest is still unspent budget.',
        },
        {
          question: 'Is a stream the same as escrow?',
          answer: 'Close, but not identical. Escrow only holds funds. A stream also defines time-based release rules and cancellation behavior.',
        },
      ],
    },
    {
      ...
      summary: 'How the product uses HTTP 402 as the machine-readable paywall and why that matters for agents.',
      plainEnglish:
      ...
      tables: [
        {
          ...
          rows: [
            ['X-FlowPay-Mode', 'Whether the route expects streaming, direct payment, or a hybrid path'],
            ['X-FlowPay-Rate', 'How much value is required'],
            ['X-FlowPay-Token', 'Which token is accepted'],
            ['X-FlowPay-Recipient', 'Which service wallet should receive value'],
            ['X-FlowPay-Contract', 'Which stream contract the runtime should use'],
          ],
        },
        {
      ...
      faqs: [
        {
          ...
          answer: 'No. For paid routes, 402 is the correct response. It is the payment negotiation step, not a bug.',
        },
        {
          question: 'Why not just use API keys?',
          answer: 'API keys identify a client. They do not solve pricing, per-route payment terms, or onchain settlement for autonomous agents.',
        },
      ],
    },
    {
      ...
      summary: 'Why this system is built for autonomous software, not just for human dashboards.',
      plainEnglish:
      ...
      tables: [
        {
          ...
          rows: [
            ['Can they tolerate manual checkout?', 'Yes, sometimes', 'No, not repeatedly'],
            ['Can they use one-off approvals every minute?', 'Maybe', 'No, too much friction'],
            ['Do they need machine-readable pricing?', 'Helpful', 'Required'],
            ['Do they need hard spending controls?', 'Nice to have', 'Mandatory'],
          ],
        },
      ],
      faqs: [
        {
          ...
          answer: 'Not in a magical way. The model or heuristic is only deciding which settlement path is more economical or safer once the payment terms are already known.',
        },
      ],
    },
    {
      ...
      title: 'Who Does What',
      eyebrow: 'Actors',
      summary: 'The product makes more sense when you understand the roles separately: provider, payer, agent, owner, renter, verifier, and admin.',
      plainEnglish:
      ...
      tables: [
        {
          ...
          rows: [
            ['Provider', 'Sets route pricing or rental terms', 'Serve access only after payment is satisfied'],
            ['Agent / payer', 'Funds direct payments or streams', 'Stay within budget and policy'],
            ['Owner', 'Controls the asset NFT', 'Manage asset metadata and future yield rights'],
            ['Renter', 'Receives temporary access', 'Pay only for usage'],
            ['Verifier', 'Reads registry and history', 'Judge authenticity and provenance'],
            ['Admin', 'Can freeze or approve', 'Keep the system safe and compliant'],
          ],
        },
      ],
      faqs: [
        {
          ...
          answer: 'Yes. In a demo, the same wallet might be owner, payer, and admin. But the model is cleaner if you still think of those powers separately.',
        },
      ],
    },
    {
      ...
      title: 'RWA Studio',
      eyebrow: 'Assets',
      summary: 'How the rental asset side works: minting, verification, renting, and yield management.',
      plainEnglish:
      ...
      faqs: [
        {
          question: 'Does owning the NFT automatically mean you physically possess the asset?',
          answer: 'No. The NFT is the digital ownership and revenue record. Physical custody and access controls still exist in the real world.',
        },
      ],
    },
    {
      ...
      summary: 'What “productive” means and why a revenue-producing asset matters more than a static collectible.',
      plainEnglish:
      ...
      tables: [
        {
          ...
          rows: [
            ['Collectible NFT', 'Scarcity or community value', 'Verification and ownership history'],
            ['Rental RWA', 'Ownership plus recurring cash flow', 'Rental access, verification, and yield coupling'],
            ['Revenue-producing equipment', 'Usage fees over time', 'Metered revenue and transparent ownership-linked yield'],
          ],
        },
      ],
      faqs: [
    ...
    },
    {
      ...
      summary: 'How revenue rights stay attached to the NFT instead of getting stranded with an old owner.',
      plainEnglish:
      ...
      tables: [
        {
          ...
          rows: [
            ['Owner', 'NFT ownership, verification record, long-term yield rights'],
            ['Renter', 'Temporary physical access through payment streaming'],
            ['Verifier', 'Authenticity checks through registry, metadata, and activity history'],
            ['Issuer/Admin', 'Compliance, freeze controls, and registry management'],
          ],
        },
      ],
      faqs: [
    ...
    },
    {
      ...
      title: 'Verification',
      eyebrow: 'Trust Layer',
      summary: 'How QR, NFC, IPFS, and the registry work together to prove authenticity and history.',
      plainEnglish:
      ...
      tables: [
        {
          ...
          rows: [
            ...
            ['Token id + optional tag seed', 'Internal review when the token is already known'],
          ],
        },
      ],
      faqs: [
      ...
      tables: [
        ...
        },
        {
          ...
          rows: [
            ...
            ['FlowPayAssetRegistry', 'Store CID hash, tag hash, issuer, and active stream mapping'],
            ['FlowPayAssetStream', 'Handle asset-linked yield and flash advance behavior'],
            ['FlowPayComplianceGuard', 'Store compliance and freeze state'],
            ['FlowPayRWAHub', 'Coordinate minting, funding, claims, and admin actions'],
          ],
        },
        {
          ...
          rows: [
            ['GET /api/engine/catalog', 'Returns runtime and route configuration'],
            ['POST /api/rwa/ipfs/metadata', 'Pins metadata and returns CID / URI'],
            ['POST /api/rwa/assets', 'Mints a new asset and verification payload'],
            ['GET /api/rwa/assets/:tokenId', 'Reads asset detail'],
            ['GET /api/rwa/assets/:tokenId/activity', 'Reads indexed asset activity'],
      ...
      tables: [
        {
          ...
          rows: [
            ['x402', 'The HTTP 402 payment handshake that says a route is paid and explains how to pay'],
            ['Payment stream', 'A budget that releases value over time instead of paying everything at once'],
            ['Agentic streaming', 'Streaming designed so software agents can use paid services repeatedly without repeated checkout'],
            ['RWA', 'A real-world asset represented by an onchain digital twin'],
            ['Productive asset', 'An asset that generates cash flow, such as rent or usage fees'],
            ['CID', 'The content fingerprint for a file or metadata object on IPFS'],
            ['Verification tag', 'The QR or NFC-linked identifier checked against the registry'],
            ['Yield coupling', 'The rule that future asset-linked revenue follows current NFT ownership'],
            ['Claimable balance', 'How much value can be withdrawn right now'],
            ['Flash advance', 'Taking some future yield early and letting time catch up later'],
          ],
        },
      ],
      faqs: [
        {
          ...
          answer: 'Remember this sentence: x402 tells the client that payment is required, and Stream Engine makes that payment reusable through streams so agents and rental flows stay economical.',
        },
      ],
    },
  ];
...
function StatusBanner({ isLoading, error }) {
  if (isLoading) {
    return (
      <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 px-4 py-3 text-sm text-cyan-100">
        Loading live runtime data for the docs…
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-sm text-amber-100">
        Live catalog could not be loaded. The page is showing safe local defaults.
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-sm text-emerald-100">
      Live runtime values are loaded from the backend catalog.
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`w-full rounded-2xl border px-4 py-3 text-left transition-colors ${
        isActive
          ? 'border-flowpay-500/30 bg-flowpay-500/15'
          : 'border-white/10 bg-white/[0.03] hover:border-white/20 hover:bg-white/[0.05]'
      }`}
    >
      <div className="flex items-start gap-3">
        <div className={`mt-0.5 rounded-xl border p-2 ${isActive ? 'border-flowpay-500/30 bg-flowpay-500/10 text-cyan-300' : 'border-white/10 bg-white/5 text-white/60'}`}>
          <Icon className="h-4 w-4" />
        </div>
        <div className="min-w-0">
          <div className="text-sm font-semibold text-white">{section.title}</div>
          <div className="mt-1 text-xs leading-relaxed text-white/45">{section.summary}</div>
        </div>
      </div>
    </button>
  );
...
function PlainLanguageCard({ text }) {
  return (
    <div className="rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-5">
      <div className="text-xs uppercase tracking-[0.22em] text-cyan-200">Explain It Like I Am New</div>
      <p className="mt-3 text-sm leading-7 text-cyan-50">{text}</p>
    </div>
  );
}
...
function TakeawayPanel({ items = [] }) {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
      <div className="text-xs uppercase tracking-[0.22em] text-cyan-300">If You Only Remember Three Things</div>
      <div className="mt-4 space-y-3">
        {items.map((item, index) => (
          <div key={item} className="flex gap-3">
            <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-flowpay-500/30 bg-flowpay-500/10 text-xs font-semibold text-cyan-300">
              {index + 1}
            </div>
      ...
      <div className="mt-4 space-y-3">
        {steps.map((step, index) => (
          <div key={step} className="flex gap-3">
            <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-flowpay-500/15 text-xs font-semibold text-cyan-300">
              {index + 1}
            </div>
            <p className="text-sm leading-7 text-white/60">{step}</p>
          </div>
        ))}
      </div>
    </div>
...
function CodeCard({ code }) {
  return (
    <div className="overflow-x-auto rounded-3xl border border-white/10 bg-black/30 p-5">
      <div className="text-xs uppercase tracking-[0.22em] text-white/35">Formula</div>
      <pre className="mt-3 whitespace-pre-wrap text-sm leading-7 text-cyan-200">{code}</pre>
    </div>
  );
}
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-white/10">
              {headers.map((header) => (
                <th key={header} className="px-5 py-3 text-left text-[11px] uppercase tracking-[0.2em] text-white/35">
                  {header}
                </th>
              ))}
            </tr>
              ...
              <tr key={`${title}-${rowIndex}`} className="border-b border-white/5 last:border-0">
                {row.map((cell, cellIndex) => (
                  <td
                    ...
                    className={`px-5 py-4 text-sm leading-7 ${cellIndex === 0 ? 'font-medium text-white/80' : 'text-white/55'}`}
                  >
                    {cell}
                  </td>
                ))}
      ...
      <div className="sticky top-0 z-50 border-b border-surface-700 bg-surface-900/90 px-4 backdrop-blur">
        <div className="mx-auto flex h-14 w-full max-w-7xl items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-white">Stream Engine Handbook</span>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto flex w-full max-w-7xl gap-8 px-4 py-8">
        <aside className="hidden w-72 shrink-0 lg:block">
          <div className="sticky top-20 space-y-4">
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
              <div className="text-xs uppercase tracking-[0.22em] text-cyan-300">Docs Map</div>
              <div className="mt-2 text-xl font-semibold text-white">Understand every moving part</div>
              <p className="mt-3 text-sm leading-7 text-white/55">
                This page explains the full product model in plain language: payment negotiation, stream settlement, agent flows, RWAs, verification, and contracts.
              </p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-[32px] border border-white/10 bg-white/[0.03] p-6 md:p-8">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="max-w-3xl">
                  <div className="text-xs uppercase tracking-[0.24em] text-cyan-300">{activeSection.eyebrow}</div>
                  <h1 className="mt-3 text-3xl font-black tracking-tight text-white md:text-4xl">{activeSection.title}</h1>
                  <p className="mt-4 max-w-2xl text-sm leading-8 text-white/60 md:text-base">
                    {activeSection.summary}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                  <div className="text-[11px] uppercase tracking-[0.22em] text-white/35">Current section</div>
                  <div className="mt-2 text-sm font-medium text-white">{activeSection.title}</div>
                </div>
              </div>

              <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                <StatCard label="Network" value={catalog?.network?.name || 'Westend Asset Hub'} />
                <StatCard label="Payment Token" value={`Circle ${catalog?.payments?.tokenSymbol || 'USDC'}`} />
                <StatCard label="Asset Id" value={String(catalog?.payments?.paymentAssetId || 31337)} />
                <StatCard label="Paid Routes" value={String(routeCount)} />
                <StatCard label="Contracts Wired" value={String(configuredContractCount)} />
              </div>
            </div>
            ...
            <div className="lg:hidden">
              <select
                ...
                onChange={(event) => navigate(`/app/docs/${event.target.value}`)}
            ...
            {activeSection.steps?.length ? (
              <StepList title={activeSection.stepsTitle || 'How it works'} steps={activeSection.steps} />
            ) : null}
            ...
            {activeSection.faqs?.length ? (
              <div className="space-y-4">
                <div className="text-xs uppercase tracking-[0.24em] text-cyan-300">Common Questions</div>
