...
function isSupportedRecipient(value, isStellarRuntime) {
  const candidate = normalizeRecipient(value);
  if (!candidate) {
    return false;
  }

  return isStellarRuntime
    ? StrKey.isValidEd25519PublicKey(candidate)
    : false;
}

function normalizeRecipientAddress(value, isStellarRuntime) {
  const candidate = normalizeRecipient(value);

  if (isStellarRuntime) {
    if (!StrKey.isValidEd25519PublicKey(candidate)) {
      throw new Error('Enter a valid Stellar public key (`G...`).');
    }
    return candidate;
  }

  throw new Error('Only Stellar public keys are supported.');
}
...
const RecipientInput = ({ value, onChange }) => {
  const isStellarRuntime = true;
  const isResolved = isSupportedRecipient(value, isStellarRuntime);
...
export default function CreateStreamForm({
  ...
  const isRecipientValid = isSupportedRecipient(recipient, isStellarRuntime);
          ...
          >
            {currentStep === 3
              ? isProcessing
                ...
                : <><Rocket className="w-4 h-4" /> Open Session</>
              : 'Continue →'}
          </button>
        </div>
      </form>
